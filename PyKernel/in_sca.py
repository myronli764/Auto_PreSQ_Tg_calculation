...
'''
x = np.random.random((9, 12, 3))*100  # n_frames, n_particles, n_dim
t = np.arange(0,10).reshape(1,10)
# traj, q mod, [2pi/Lx, 2pi/Ly, 2pi/Lz], rtol -> returns fq and q vectors
ret = incoherrent_scattering(x, 6.3, [2*np.pi/100,2*np.pi/100, 2*np.pi/100], rtol=1e-2)
#plt.plot(t,ret[0])
print(ret[0].shape, ret[1].shape)
print(ret[1])
# traj, q vectors
'''
#print(incoherrent_scattering(x, q_vectors=ret[1]))
